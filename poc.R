...
clean_data <- function(gc_data, geo_data) {
  # Assuming gc and geo are data frames with appropriate columns
    #  rename columns for clarity # nolint: indentation_linter.
    ...
    #  But for now I willremove the rows with NA in Lat and Long
...
xy <- sp::SpatialPointsDataFrame(
  matrix(c(data$LAT, data$LONG), ncol=2), data.frame(ID=seq(1:length(data$LAT))),
...
sys.time(res <- dbscan::hdbscan(mdist, minPts = 10))
plot(res)
