---
title: "HDBSCAN Bristol"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{HDBSCAN Bristol}
...
This vignette demonstrates how to create interactive maps using the `leaflet` package in R to visualize the gambling neighbourhoods identified using an hybrid approach combining HDBSCAN clustering with a fixed eps=500m specifically for Bristol. 
...
📝 **Note:** Results for MinPts=2 and 3 are not shown here as they produced a very high number of clusters (over 100) which are difficult to visualise and interpret on a map.

<br>

## Load data
Load the geographical data (spatial points dataframe) containing the coordinates of gambling premises in Bristol.

```{r load-data, echo=TRUE, message=FALSE}
xy_bristol <- readRDS("data/xy_bristol.rds")
```
## MinPts=4 
```{r k4_eps500, message=FALSE}
res4_eps500 <- readRDS("results/hdbscan_results_eps500_MinPts4_bristol.rds")
# Create a data frame from the spatial points
df4_eps500 <- create_df(res4_eps500,xy_bristol)
head(df4_eps500)
dim(df4_eps500)
```
...
res5_eps500 <- readRDS("results/hdbscan_results_eps500_MinPts5_bristol.rds")
# Create a data frame from the spatial points
df5_eps500 <- create_df(res5_eps500,xy_bristol)
head(df5_eps500)
dim(df5_eps500)   
```
Number of clusters (excluding noise):
