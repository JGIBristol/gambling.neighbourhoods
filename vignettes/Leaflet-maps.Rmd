...
```{r k2, message=FALSE}
res2 <- readRDS("data/hdbscan_results_2.rds")
...
# saveWidget(map2, file="data/map_MinPt2.html")
map2
...
Load dataframe with coordinates and cluster assignments
```{r k3, message=FALSE}
res3 <- readRDS("data/hdbscan_results_3.rds")
# Create a data frame from the spatial points
...
# saveWidget(map3, file="data/map_MinPt3.html")
map3
``` 
## MinPts=4
Load dataframe with coordinates and cluster assignments
```{r k4, message=FALSE}
res4 <- readRDS("data/hdbscan_results_4.rds")
# Create a data frame from the spatial points
...
Load dataframe with coordinates and cluster assignments
```{r k5, message=FALSE}
res5 <- readRDS("data/hdbscan_results_5.rds")
# Create a data frame from the spatial points
...
#saveWidget(map5, file="data/map_MinPt5.html")
map5
``` 
...
Load dataframe with coordinates and cluster assignments
```{r k10, message=FALSE}
res10 <- readRDS("data/hdbscan_results_10.rds")
# Create a data frame from the spatial points
...
# saveWidget(map10, file="data/map_MinPt10.html")
map10
```


## Summary of number of clusters for each MinPt
...
knitr::kable(tab, caption = "For each MinPts value, summary of number of clusters, number of noise points (unclustered), maximum and minimum number of gambling premises per cluster.")
```
